...
ただし、Dart版とは異なり、DeltaTraceDatabase クラスの execute_query_object、execute_query、
および execute_transaction_query は内部で RLock を取得するため、これらのクエリ実行部分に関してはマルチスレッドでの呼び出しが可能です。
ただし、その他のクラスやユーティリティ関数はスレッドセーフではないため、並行して使用する場合は注意が必要です。  
